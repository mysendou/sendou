...
public class OrderDao {
	...
	public PageBean<SellItemCategory> findAllCategoryBook(int pc) throws SQLException {
		return findSellByCcategory(pc);
	}
	...
	private PageBean<SellItemCategory> findSellByCcategory(int pc) throws SQLException {
        ...
        for (SellItemCategory item : list) {
            sql = "SELECT COUNT(*) FROM t_orderitem o,t_category t, t_book b WHERE o.bid=b.bid AND b.cid=t.cid AND t.cname=?";
            Number number = (Number)qr.query(sql, new ScalarHandler(),item.getCname());
            item.setNumber(number.intValue());
        }
